...
def wave_transformation(input_data, output_data, width, height):
	frequency_modifier = Decimal(random.uniform(0.6, 0.8) * (width / height)) #slight randomisation for frequency
	frequency = frequency_modifier * (Decimal(1) / Decimal((width / 2))) * Decimal(math.pi)

	amplitude_modifier = random.uniform(5, 7) #slight randomisation for amplitude
...
def find_dominant_color(input_img):
	...
	image_array = scipy.misc.fromimage(input_img) #numpy array from input image
	shape = image_array.shape
	image_array = image_array.reshape(scipy.product(shape[:2]), shape[2])

	codes, distortion = scipy.cluster.vq.kmeans(image_array.astype(float), NUM_CLUSTERS)
...
def main():
	...
	output_img = processImage(input_img)
